...
class TestLlamaServer:
  ...
  def setup_server(self, artifact_name: str, model_path: str, release_artifacts: List[str], server_instance: Callable):
    """Setup server once before all tests in this class"""
    ...
    self.server = server_instance(artifact_path, model_path)
  ...
  def test_health_check(self):
    ...
    print(f"✓ Health check passed for {self.artifact_path}")
  ...
  def test_completion(self, prompt_config):
    ...
    print(f"✓ Completion test passed for {self.artifact_path} with prompt: {prompt_config['prompt'][:20]}...")
