...
@pytest.fixture
def server_setup(model_path: str, release_artifacts: List[str], 
                server_instance: Callable, artifact_name: str):
  ...
  assert artifact_path is not None, \
    f"Artifact {artifact_name} not found in {release_artifacts}"
...
@pytest.mark.parametrize("artifact_name", SUPPORTED_ARTIFACTS)
@pytest.mark.parametrize("prompt_config", [
  {
...
def test_completion(server_setup, prompt_config):
  ...
  print(f"✓ Completion test passed for {server_setup['artifact_path']} "
        f"with prompt: {prompt_config['prompt'][:20]}...")
